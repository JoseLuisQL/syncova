...
export class ReporteExportService {
  ...
  static async exportarStockCritico(
    ...
    try {
      ...
      // Configurar columnas específicas para stock crítico
      worksheet.columns = [
        ...
        { header: '% Crítico', key: 'porcentajeCritico', width: 10 },
        { header: 'Nivel', key: 'nivelCriticidad', width: 12 },
        { header: 'Acción Recomendada', key: 'recomendacionAccion', width: 35 },
        { header: 'Lotes Disponibles', key: 'lotesDisponibles', width: 15 }
      ];
      ...
      // Aplicar estilos con colores de alerta
      this.aplicarEstilosStockCritico(worksheet);
  ...
  private static agregarDatosVencimientos(worksheet: ExcelJS.Worksheet, data: VencimientoItem[], config: ReporteExportConfig): void {
    ...
    data.forEach((item, index) => {
      ...
      row.values = [
        ...
        item.vacunaTipo,
        item.cantidadActual,
        item.fechaVencimiento.toLocaleDateString('es-PE'),
        item.diasParaVencer,
  ...
  private static aplicarEstilosStockCritico(worksheet: ExcelJS.Worksheet): void {
    this.aplicarEstilosExcel(worksheet);
    // Estilos adicionales para stock crítico se implementarán después
  }
