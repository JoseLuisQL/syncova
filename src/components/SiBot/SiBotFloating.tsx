...
import { Robot, X, DotsThreeCircle, Trash } from '@phosphor-icons/react';
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className={`flex items-center justify-center w-14 h-14 rounded-full shadow-lg transition-transform hover:scale-105 active:scale-95 ${
            isOpen ? 'bg-zinc-800' : 'bg-blue-600'
          } text-white`}
          aria-label="Toggle SiBot"
        >
          {isOpen ? <X className="w-6 h-6" /> : <Robot className="w-8 h-8" weight="fill" />}
        </button>
      </div>
              ...
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-80">
                  <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mb-4">
                    <Robot className="w-10 h-10 text-blue-600" weight="duotone" />
                  </div>
