    ...
    <div className={`${COMPONENT_STYLES.panel} inventory-loading-shell p-5`}>
      <div className="grid gap-4 xl:grid-cols-2">
        {Array.from({ length: 2 }).map((_, columnIndex) => (
          <div key={`config-nav-skeleton-${columnIndex + 1}`} className="rounded-[18px] border border-slate-200 bg-slate-50/70 p-3">
            ...
            <div className="mt-3 grid gap-2 min-[520px]:grid-cols-2">
              {Array.from({ length: 2 }).map((__, buttonIndex) => (
                <div key={`config-button-skeleton-${columnIndex + 1}-${buttonIndex + 1}`} className="rounded-2xl border border-slate-200 bg-white p-3">
