...
const StockAvailabilitySection: React.FC<StockAvailabilitySectionProps> = ({ data, isLoading }) => {
  if (isLoading) {
    return (
      <div className="h-full animate-pulse rounded-3xl border border-[#e3e9f0] bg-white p-5">
  ...
  return (
    <section className="flex h-full flex-col rounded-3xl border border-line bg-surface p-5 shadow-[0_16px_40px_-34px_rgba(15,42,59,0.55)]">
      <div className="mb-6">
        <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-muted">Inventario</p>
        <h3 className="mt-1 text-md font-semibold tracking-[-0.02em] text-ink">Disponibilidad en stock</h3>
      </div>

      <div className="mb-3 flex px-1 font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-2">
      ...
      <div className="flex-1 divide-y divide-line-soft">
        {sortedData.map((item, i) => {
          ...
          return (
            <div key={i} className="flex items-center py-3 text-base font-medium text-ink">
              ...
              <div className="flex w-1/3 items-center justify-end gap-2 sm:gap-3">
                <div className="h-1.5 w-12 flex-shrink-0 overflow-hidden rounded-full bg-surface-soft sm:w-16">
                  <div
                    className="h-full rounded-full bg-[#0e9f8e]"
                    style={{ width: `${pct}%` }}
                  />
                </div>
                <span className="min-w-[42px] whitespace-nowrap text-right font-mono text-sm text-muted-2 tabular-nums">{diasCobertura}d</span>
              </div>
            </div>
          );
        })}
