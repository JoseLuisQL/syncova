...
const StockAvailabilitySection: React.FC<StockAvailabilitySectionProps> = ({ data, isLoading }) => {
  ...
  return (
    <section className="flex h-full flex-col rounded-3xl border border-[#e3e9f0] bg-white p-5 shadow-[0_16px_40px_-34px_rgba(15,42,59,0.55)]">
      <div className="mb-6">
        <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[#9aa4b2]">Inventario</p>
        <h3 className="mt-1 text-md font-semibold tracking-[-0.02em] text-[#171b22]">Disponibilidad en stock</h3>
      </div>
      ...
      <div className="flex-1 divide-y divide-zinc-100">
        {sortedData.map((item, i) => {
          ...
          return (
            <div key={i} className="flex items-center py-3 text-base font-medium text-primary">
              ...
              <div className="flex w-1/3 items-center justify-end gap-2 sm:gap-3">
                ...
                <span className="min-w-[42px] whitespace-nowrap text-right font-mono text-sm text-secondary tabular-nums">{diasCobertura}d</span>
              </div>
            </div>
          );
        })}
